...
export function cardsFromTokens(tokens, options) {
  ...
  tokens.forEach((token, i) => {
    console.log(`Token ${i}: Type - ${token.type}, Content - "${token.content}"`);

    // Check for the special #kk token
    if (token.type === 'inline' && token.content.trim().startsWith('#kk')) {
      console.log('Found #kk token');

      // If we're not in the front yet, push the current card and reset
      if (!isFront) {
      ...
      // Handle the front of the card: Content after #kk
      let remainingContent = token.content.trim().replace('#kk', '').trim();

      // Check if the content has a split marker (%)
      if (remainingContent.includes('%')) {
        let parts = remainingContent.split('%');
        card.front.push({
          type: 'text',
          content: parts[0].trim(), // Front content before the %
        });
        card.back.push({
          type: 'text',
          content: parts[1].trim(), // Back content after the %
        });
      } else {
        // If no split marker, treat everything as front content
        card.front.push({
          type: 'text',
          content: remainingContent, // All content goes to the front
        });
      }

      // Skip the rest of the token processing for this one (since it's already processed)
      return;
    }

    // If it's not #kk, handle the token normally:
    // Handle normal token processing and pushing to front/back of the card
    if (token.type === 'heading_open' && !isFront) {
      const parent = [...cards].reverse().find(c => c.headingLevel < card.headingLevel);
      if (parent) card.setParent(parent);
      cards.push(card);
      isFront = true;
    ...
    // Handle split conditions: using % or <!-- md2apkg split -->
    if (!isFront && token.type === 'inline' && (token.content.trim().includes('<!-- md2apkg split -->') || token.content.trim() === '%')) {
      card.back.pop();
      card.front.push(...card.back);
      card.back = [];
    }

    // End the front if it's a heading close tag
    if (token.type === 'heading_close' && isFront) isFront = false;
  });

  // Ensure the last card is added if it has content
  if (card.front.length || card.back.length) {
    cards.push(card);
  }
